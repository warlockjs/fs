...
The rename step retries on transient Windows locks (`EPERM` / `EBUSY` / `EACCES` — antivirus or the search indexer briefly holding a just-created file) with bounded exponential backoff: 5 attempts, starting at 20ms and doubling, capping total wait under ~1s. Any other error is thrown immediately. It never falls back to a non-atomic direct write — if every retry fails, it throws a named `AtomicWriteError` naming the target path and the attempt count, with the last OS error attached as `cause`.
...
4. rename(tempPath, filePath), retrying on EPERM/EBUSY/EACCES with backoff
   on exhaustion: unlink(tempPath), throw AtomicWriteError
   on any other error: unlink(tempPath), rethrow immediately
```
...
try {
  ...
  if (error instanceof AtomicWriteError) {
    // error.targetPath — the file that couldn't be replaced
    // error.attempts   — how many rename attempts were made
    // error.cause      — the last OS error (e.g. { code: "EBUSY" })
  }
  throw error;
}
```

`AtomicWriteError` is thrown only after every retry attempt fails on a transient lock code. A non-retryable error (e.g. `ENOSPC`, or the target being an existing directory) is thrown immediately, unwrapped, after a single attempt — it is never wrapped in `AtomicWriteError`.
